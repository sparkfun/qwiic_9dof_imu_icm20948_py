...
class QwiicIcm20948(object):
	...
	def setBank(self, bank):
		""" 
			设置ICM20948模块的bank寄存器

			:return: 如果bank是有效值并且已经设置，返回True，否则返回False。
			:rtype: bool

		"""
		if bank > 3:	# 只有4个可能的banks
			print("无效的Bank值: %d" % bank)
			return False			   
		bank = ((bank << 4) & 0x30) # REG_BANK_SEL的位5:4
		#return ICM_20948_execute_w(pdev, REG_BANK_SEL, &bank, 1)
		return self._i2c.writeto_mem(self.address, self.REG_BANK_SEL, bytes([bank]))
	...
	def swReset(self):
		...
		self.setBank(0)
		register = self._i2c.readfrom_mem(self.address, self.AGB0_REG_PWR_MGMT_1, 1)[0]
		...
		# Write register
		self.setBank(0)
		return self._i2c.writeto_mem(self.address, self.AGB0_REG_PWR_MGMT_1, bytes([register]))		
	...
	def setSampleMode(self, sensors, mode):
		...
		self.setBank(0)
		register = self._i2c.readfrom_mem(self.address, self.AGB0_REG_LP_CONFIG, 1)[0]
		...
		# Write register
		self.setBank(0)
		return self._i2c.writeto_mem(self.address, self.AGB0_REG_LP_CONFIG, bytes([register]))		
	...
	def setFullScaleRangeAccel(self, mode):
		...
		self.setBank(2)
		register = self._i2c.readfrom_mem(self.address, self.AGB2_REG_ACCEL_CONFIG_1, 1)[0]
		...
		# Write register
		self.setBank(2)
		return self._i2c.writeto_mem(self.address, self.AGB2_REG_ACCEL_CONFIG_1, bytes([register]))	
	...
	def setFullScaleRangeGyro(self, mode):
		...
		self.setBank(2)
		register = self._i2c.readfrom_mem(self.address, self.AGB2_REG_GYRO_CONFIG_1, 1)[0]
		...
		# Write register
		self.setBank(2)
		return self._i2c.writeto_mem(self.address, self.AGB2_REG_GYRO_CONFIG_1, bytes([register]))			
	...
	def dataReady(self):
		...
		self.setBank(0)
		register = self._i2c.readfrom_mem(self.address, self.AGB0_REG_INT_STATUS_1, 1)[0]
	...
	# getAgmt()
	#
	# 从ICM90248模块的加速度计，陀螺仪，磁力计和温度读取并更新原始值
	def getAgmt(self):
		""" 
			从ICM90248模块的加速度计，陀螺仪，磁力计和温度读取并更新原始值

			:return: 如果I2C readfrom_mem成功，则返回True，否则返回False。
			:rtype: bool

		"""

		# 从AGB0_REG_ACCEL_XOUT_H开始读取所有的读数
		numbytes = 14 + 9 # 读取加速度计，陀螺仪，温度和9字节的磁力计
		self.setBank(0)
		buff = self._i2c.readfrom_mem(self.address, self.AGB0_REG_ACCEL_XOUT_H, numbytes)
	...
	def i2cMasterPassthrough(self, passthrough):
		...
		self.setBank(0)
		register = self._i2c.readfrom_mem(self.address, self.AGB0_REG_INT_PIN_CONFIG, 1)[0]
		...
		# Write register
		self.setBank(0)
		return self._i2c.writeto_mem(self.address, self.AGB0_REG_INT_PIN_CONFIG, bytes([register]))	
	...
	def i2cMasterEnable(self, enable):
		...
		self.setBank(3)
		register = self._i2c.readfrom_mem(self.address, self.AGB3_REG_I2C_MST_CTRL, 1)[0]
		...
		# Write register
		self.setBank(3)
		self._i2c.writeto_mem(self.address, self.AGB3_REG_I2C_MST_CTRL, bytes([register]))

		# enable/disable Master I2C
		# Read the AGB0_REG_USER_CTRL, store in local variable "register"
		self.setBank(0)
		register = self._i2c.readfrom_mem(self.address, self.AGB0_REG_USER_CTRL, 1)[0]
		...
		# Write register
		self.setBank(0)
		return self._i2c.writeto_mem(self.address, self.AGB0_REG_USER_CTRL, bytes([register]))
	...
	def ICM_20948_i2c_master_slv4_txn(self, addr, reg, data, Rw, send_reg_addr):
		...
		self.setBank(3)
		self._i2c.writeto_mem(self.address, self.AGB3_REG_I2C_SLV4_ADDR, bytes([addr]))

		self.setBank(3)
		self._i2c.writeto_mem(self.address, self.AGB3_REG_I2C_SLV4_REG, bytes([reg]))
		...
		if (Rw == False):
			self.setBank(3)
			self._i2c.writeto_mem(self.address, self.AGB3_REG_I2C_SLV4_DO, bytes([data]))

		# Kick off txn
		self.setBank(3)
		self._i2c.writeto_mem(self.address, self.AGB3_REG_I2C_SLV4_CTRL, bytes([ctrl_register_slv4]))
		...
		while (slave4Done == False):
			self.setBank(0)
			i2c_mst_status = self._i2c.readfrom_mem(self.address, self.AGB0_REG_I2C_MST_STATUS, 1)[0]
		...
		if Rw:
			self.setBank(3)
			return self._i2c.readfrom_mem(self.address, self.AGB3_REG_I2C_SLV4_DI, 1)[0]
	...
	def i2cMasterConfigureSlave(self, slave, addr, reg, len, Rw, enable, data_only, grp, swap):
		...
		self._i2c.writeto_mem(self.address, slv_addr_reg, bytes([address]))

		# Set the slave sub-address (reg)
		subAddress = reg
		self._i2c.writeto_mem(self.address, slv_reg_reg, bytes([subAddress]))
		...
		ctrl_reg_slvX |= (grp << 4)
		return self._i2c.writeto_mem(self.address, slv_ctrl_reg, bytes([ctrl_reg_slvX]))
	...
	def begin(self):
		...
		self.setBank(0)
		chipID = self._i2c.readfrom_mem(self.address, self.AGB0_REG_WHO_AM_I, 1)[0]
